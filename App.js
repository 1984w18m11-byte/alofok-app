  ...
  <Pressable style={s.prayerLink} onPress={()=>changeTab('adhan')}>
   ...
   {tab==='education'&&<>
    ...
    <Card title='حلقات وشروحات مرتبة'>
     <Text style={s.researchNotice}>الروابط التالية تفتح YouTube خارج التطبيق. وُصفت كرابط نتائج بحث حتى تبقى المواد المتاحة والمصادر الأصلية ظاهرة بوضوح، ولا ننسب عنوان حلقة غير موثّق.</Text>
     {EDUCATIONAL_LINKS.map((item,index)=><Pressable key={item.url} style={s.educationLink} onPress={()=>openEducationalLink(item.url)}>
      ...
      <View style={s.educationLinkBody}><Text style={s.educationLinkTitle}>{item.title}</Text><Text style={s.educationLinkDescription}>{item.description}</Text><Text style={s.educationLinkAction}>فتح في YouTube ←</Text></View>
     </Pressable>)}
    </Card>
   </>}
   {tab==='calendar'&&<>

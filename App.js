...
{tab==='settings'&&<>
    ...
    {IS_PLUS&&<SettingsCard title={ui('ثيمات خلفية الجهاز','Device Wallpaper Themes')}>
     ...
     <Pressable style={s.secondaryButton} onPress={()=>Platform.OS==='android'?Linking.sendIntent('android.settings.WALLPAPER_SETTINGS').catch(()=>Linking.openSettings()):Alert.alert('الآيفون','احفظ الصورة وطبّقها يدوياً أو بواسطة تطبيق الاختصارات.')}><Text style={s.secondaryButtonText}>{ui('فتح إعدادات خلفية الجهاز','Open device wallpaper settings')}</Text></Pressable>

...
{tab==='settings'&&<>
    ...
    <SettingsCard title={t('appLanguage')}>
     <Pressable style={s.soundSelector} onPress={()=>setShowLanguageChoices(v=>!v)}>
      ...
      <View style={s.settingText}><Text style={s.text}>{t('chooseLanguage')}</Text><Text style={s.sub}>{LANGUAGE_CHOICES.find(([id])=>id===appLanguage)?.[1]||t('deviceLanguage')}</Text></View>
    ...
    {IS_PLUS&&<SettingsCard title={ui('متجر الإضافات','Add-ons Store')}>
     <Text style={s.text}>{ui('مكان مخصص مستقبلاً لشراء ثيمات وأصوات وحزم موسمية بصورة منفردة.','Reserved for future individual purchases of themes, sounds and seasonal packs.')}</Text>
     <View style={s.shopRow}><View style={s.shopItem}><Text style={s.shopIcon}>◐</Text><Text style={s.shopText}>{ui('ثيمات','Themes')}</Text></View><View style={s.shopItem}><Text style={s.shopIcon}>♪</Text><Text style={s.shopText}>{ui('أصوات','Sounds')}</Text></View><View style={s.shopItem}><Text style={s.shopIcon}>✦</Text><Text style={s.shopText}>{ui('حزم موسمية','Seasonal packs')}</Text></View></View>
    </SettingsCard>}
    <SettingsCard title={t('updates')}>
     <Pressable style={[s.updateButton,updateInfo&&s.updateButtonAvailable]} onPress={()=>updateInfo?showUpdateDialog():checkForUpdate(true)}>
      ...
      <View style={s.updateTextWrap}>
       <Text style={s.updateTitle}>{updateInfo?ui(`تحديث جديد — الإصدار ${updateInfo.version}`,`New update — version ${updateInfo.version}`):t('checkUpdate')}</Text>
       <Text style={s.updateSub}>{updateChecking?t('checking'):updateInfo?ui('اضغط لعرض التفاصيل والتحديث','Tap to view details and update'):ui(`الإصدار الحالي ${APP_VERSION}`,`Current version ${APP_VERSION}`)}</Text>
      </View>
      <Text style={s.chevron}>‹</Text>
     </Pressable>
     {lastUpdateCheck&&<Text style={s.lastCheck}>{ui('آخر فحص: ','Last check: ')}{lastUpdateCheck.toLocaleTimeString(useArabicUi?'ar-IQ':'en-US',{hour:'2-digit',minute:'2-digit'})}</Text>}
    </SettingsCard>
    <SettingsCard title={t('alertSettings')}>
     ...
     <Pressable style={s.soundSelector} onPress={()=>{if(showAdhanVoices)stopAdhan();setShowAdhanVoices(v=>!v)}}><Text style={s.settingIcon}>🔊</Text><View style={s.settingText}><Text style={s.text}>{t('adhanSound')}</Text><Text numberOfLines={1} style={s.sub}>{selectedAdhan?(useArabicUi?selectedAdhan.display_ar:(selectedAdhan.performer||selectedAdhan.display_ar)):t('chooseAdhan')}</Text></View><Text style={s.chevron}>‹</Text></Pressable>
     {showAdhanVoices&&packs.map(p=><Pressable key={p.id} style={[s.adhanChoice,selectedAdhan?.id===p.id&&s.selectedSound]} onPress={async()=>{setSelectedAdhan(p);await AsyncStorage.setItem('alofq_selected_adhan_id',p.id);await AsyncStorage.setItem('alofq_adhan_manual','1');previewAdhan(p)}}><Text style={s.playIcon}>▶</Text><Text style={s.text}>{useArabicUi?p.display_ar:(p.performer||p.display_ar)}</Text></Pressable>)}
     {showAdhanVoices&&<Pressable disabled={!adhanSound} style={[s.stopButton,!adhanSound&&s.stopButtonDisabled]} onPress={stopAdhan}><Text style={s.stopButtonText}>■ {ui('إيقاف المعاينة','Stop preview')}</Text></Pressable>}
     <View style={s.volumeBox}><Text style={s.text}>{ui('مستوى الصوت','Volume')} — {Math.round(adhanVolume*100)}%</Text><View style={s.volumeTrack}>{[.2,.4,.6,.8,1].map(level=><Pressable key={level} onPress={()=>changeAdhanVolume(level)} style={[s.volumeStep,adhanVolume>=level&&s.volumeStepOn]}/>)}</View></View>
     {selectedAdhan&&<><Pressable style={s.secondaryButton} onPress={()=>setShowAdhanLicense(v=>!v)}><Text style={s.secondaryButtonText}>{showAdhanLicense?ui('إخفاء ترخيص الصوت','Hide sound license'):ui('عرض ترخيص الصوت المختار','Show selected sound license')}</Text></Pressable>{showAdhanLicense&&<View><Text style={selectedAdhan.status==='licensed'?s.ok:s.warn}>{selectedAdhan.status==='licensed'?ui('✓ الترخيص: '+(selectedAdhan.license||'غير محدد')+' • المصدر: '+(selectedAdhan.source||'غير محدد'),'✓ License: '+(selectedAdhan.license||'Not specified')+' • Source: '+(selectedAdhan.source||'Not specified')):ui((selectedAdhan.note_ar||'هذا التسجيل يحتاج إلى إثبات تصريح قبل إضافته للتطبيق.'),'This recording requires documented permission before it can be included in the app.')}</Text>{selectedAdhan.source_url&&<Pressable onPress={()=>Linking.openURL(selectedAdhan.source_url)}><Text style={s.licenseLink}>{ui('فتح صفحة المصدر والترخيص','Open source and license page')}</Text></Pressable>}</View>}</>}
     {isRamadan&&<View style={s.volumeBox}><Text style={s.text}>{ui('الإمساك','Imsak')}: {fasting.imsak}  •  {ui('الإفطار','Iftar')}: {fasting.iftar}</Text><Text style={[s.text,{marginTop:12,lineHeight:28}]}>{ui(RAMADAN_VERSE,RAMADAN_VERSE_EN)}</Text><Text style={s.sub}>{ui('سورة البقرة — الآية 187','Surah Al-Baqarah — verse 187')}</Text></View>}
    </SettingsCard>
    <SettingsCard title={ui('شرح التقويم العربي','Arabic Calendar Research')}>
    ...
    {IS_PLUS&&<SettingsCard title={ui('ثيمات خلفية الجهاز','Device Wallpaper Themes')}>
     ...
     <Text style={s.sub}>{ui('طريقة التغيير','Change mode')}</Text>
     <View style={s.wrap}>{[['time',ui('حسب وقت اليوم','By time of day')],['lunar',ui('حسب الشهر القمري','By lunar month')],['season',ui('حسب الفصل','By season')],['fixed',ui('ثيم ثابت','Fixed theme')]].map(([id,label])=><Pressable key={id} style={[s.choice,wallpaperMode===id&&s.choiceOn]} onPress={async()=>{setWallpaperMode(id);await setWallpaperPreference('alofq_wallpaper_mode',id)}}><Text style={wallpaperMode===id?s.choiceOnText:s.choiceText}>{label}</Text></Pressable>)}</View>
     <Text style={s.sub}>{ui('مكان الخلفية','Wallpaper target')}</Text>
     <View style={s.wrap}>{[['home',ui('الشاشة الرئيسية','Home screen')],['lock',ui('شاشة القفل','Lock screen')],['both',ui('الاثنتان','Both')]].map(([id,label])=><Pressable key={id} style={[s.choice,wallpaperTarget===id&&s.choiceOn]} onPress={async()=>{setWallpaperTarget(id);await setWallpaperPreference('alofq_wallpaper_target',id)}}><Text style={wallpaperTarget===id?s.choiceOnText:s.choiceText}>{label}</Text></Pressable>)}</View>
     {(wallpaperFallback||wallpaperMode==='fixed')&&<Text style={s.warn}>{ui('وضع الخلفية الثابتة: اختر ثيمًا من الأعلى ثم افتح نافذة النظام لتطبيقه.','Fixed wallpaper mode: choose a theme above, then open the system screen to apply it.')}</Text>}
     <Pressable style={s.secondaryButton} onPress={()=>Platform.OS==='android'?Linking.sendIntent('android.settings.WALLPAPER_SETTINGS').catch(()=>Linking.openSettings()):Alert.alert(ui('الآيفون','iPhone'),ui('احفظ الصورة وطبّقها يدوياً أو بواسطة تطبيق الاختصارات.','Save the image and apply it manually or with the Shortcuts app.'))}><Text style={s.secondaryButtonText}>{ui('فتح إعدادات خلفية الجهاز','Open device wallpaper settings')}</Text></Pressable>
     <Text style={s.policyMeta}>{ui('قد تختلف خيارات الشاشة الرئيسية والقفل حسب الشركة وإصدار أندرويد. لا يتجاوز الأفق قرار النظام.','Home and lock screen options may vary by manufacturer and Android version. AlofoK does not override system restrictions.')}</Text>
    </SettingsCard>}
    <SettingsCard title={t('city')}><Pressable style={s.secondaryButton} onPress={()=>setShowCityChoices(v=>!v)}><Text style={s.secondaryButtonText}>{showCityChoices?t('closeCities'):t('showCities')}</Text></Pressable>{showCityChoices&&cities.map(item=><Pressable key={item.id} style={[s.cityChoice,item.id===city?.id&&s.cityChoiceActive]} onPress={()=>{chooseCity(item.id);setShowCityChoices(false)}}><Text style={item.id===city?.id?s.cityChoiceTextActive:s.cityChoiceText}>{useArabicUi?item.name_ar:(item.name_en||item.name_ar)}</Text></Pressable>)}</SettingsCard>
    {APP_VARIANT==='trial'&&<SettingsCard title={ui('الإعلان في تطبيق الأفق','Advertising in AlofoK')}>
     <Text style={s.text}>{ui('مساحة لإعلان واحد في الأسبوع. لا يظهر أي إعلان عندما لا توجد حملة فعّالة.','Space for one advertisement per week. No ad appears when there is no active campaign.')}</Text>
     <Pressable style={s.secondaryButton} onPress={contactForAdvertising}><Text style={s.secondaryButtonText}>{ui('أعلن في تطبيق الأفق','Advertise in AlofoK')}</Text></Pressable>
     <Text style={s.sub}>{ui('التواصل عبر واتساب أو الاتصال سيُفعّل بعد إضافة رقم الإعلانات.','WhatsApp or phone contact will be enabled after an advertising contact number is added.')}</Text>
    </SettingsCard>}
    <SettingsCard title={t('privacy')}>
     <Text style={s.text}>{ui('الموقع لحساب المواقيت فقط، والتفضيلات محفوظة على جهازك.','Location is used only to calculate times. Your preferences are stored on your device.')}</Text>
     <Pressable style={s.secondaryButton} onPress={()=>setShowPrivacy(v=>!v)}><Text style={s.secondaryButtonText}>{showPrivacy?ui('إخفاء السياسة','Hide policy'):ui('قراءة سياسة الخصوصية','Read Privacy Policy')}</Text></Pressable>
     {showPrivacy&&<><Text style={s.policyText}>{ui(PRIVACY_SUMMARY,PRIVACY_SUMMARY_EN)}</Text><Text style={s.policyMeta}>{ui('سارية على النسختين • آخر تحديث: 6 سبتمبر 2026','Applies to both editions • Last updated: September 6, 2026')}</Text></>}
    </SettingsCard>
    <SettingsCard title={t('copyright')}>
     <Text style={s.text}>{ui('© 2026 الأفق — تصميم وفكرة وتطوير: وسام محمد','© 2026 AlofoK — Design, concept and development: Wisam Mohammed')}</Text>
     <Pressable style={s.secondaryButton} onPress={()=>setShowCopyright(v=>!v)}><Text style={s.secondaryButtonText}>{showCopyright?ui('إخفاء السياسة','Hide policy'):ui('قراءة سياسة الطبع والتوزيع','Read Copyright and Distribution Policy')}</Text></Pressable>
     {showCopyright&&<><Text style={s.policyText}>{ui(COPYRIGHT_SUMMARY,COPYRIGHT_SUMMARY_EN)}</Text><Text style={s.policyMeta}>{ui('المكونات الخارجية تبقى خاضعة لتراخيص أصحابها.','External components remain subject to their owners’ licenses.')}</Text></>}
    </SettingsCard>
   </>}
  </ScrollView>
  </SafeAreaView>

 ...
 async function useGps(showMessage=true){
   try{
     ...
     try{
       ...
       const g=r?.[0];
       label=[g?.country,g?.region,g?.city,g?.subregion,g?.district,g?.name]
         .filter(Boolean)
         .join(' • ')||nearest.name_ar;
     }catch(e){
       label=nearest.name_ar;
     }

     setCity(nearest);
     setLocState(`${label} • GPS (${c.lat.toFixed(4)}, ${c.lon.toFixed(4)})`);
   }catch(e){

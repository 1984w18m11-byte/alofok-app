...
assert(paymentTransfer.includes('visibleNumber(CARD_NUMBER)'),'16-digit transfer number must be visible in the compact activation panel');
assert(paymentTransfer.includes('visibleNumber(ACCOUNT_NUMBER)'),'10-digit account number must be visible in the compact activation panel');
assert(!paymentTransfer.includes('maskedLast4'),'payment numbers must not be reduced to last four digits in this test build');
...
assert(!app.includes("from './AdhkarScreen'")&&!app.includes("from './AdhkarWidgets'")&&!app.includes("from './useAdhkar'"),'adhkar must remain disabled in the current Trial test build');
assert(!app.includes('adhkarMorning')&&!app.includes('adhkarEvening')&&!app.includes('AdhkarHomeCard')&&!app.includes('AdhkarSettings'),'adhkar routes, home card and settings must remain absent in the current Trial test build');
...
assert(trial.versionCode===1001102&&trial.versionCode>=1000000&&trial.versionCode<2000000,'Trial manifest must stay in the Trial versionCode range');
...
assert(config.includes('const androidVersionCode = isPaid ? 2001101 : 1001102;'),'variant-specific Android versionCode ranges missing');
...
console.log('Release QA passed for ALAUFUQ V3',version);

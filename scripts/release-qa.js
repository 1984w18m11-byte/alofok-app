...
assert(app.includes('adhkar.schedule()'),'adhkar reminder scheduling must be active');
assert(adhkarFeature.includes("now.getTime()>=fajrDate.getTime()")&&adhkarFeature.includes('localMinutes<600'),'morning adhkar must begin after Fajr and expire at 10:00 AM');
assert(adhkarFeature.includes('localMinutes>=1260&&localMinutes<1440'),'evening adhkar must run from 9:00 PM until midnight');
assert(adhkarFeature.includes("DONE_KEY")&&adhkarFeature.includes('markComplete'),'completed adhkar must stay hidden for the current period');
assert(adhkarFeature.includes('morningAlert')&&adhkarFeature.includes('eveningAlert')&&adhkarFeature.includes('setMorningAlert')&&adhkarFeature.includes('setEveningAlert'),'independent morning/evening reminder switches must exist');

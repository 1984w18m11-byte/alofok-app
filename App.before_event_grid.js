...
const religiousFor=(m,d)=>religiousEvents.filter(e=>(e.m===m&&e.d===d)||(e.m===m&&e.range&&d>=e.range[0]&&d<=e.range[1])||(e.dates||[]).some(x=>x.m===m&&x.d===d));
...
export default function App(){
 ...
 return <SafeAreaView style={s.root}>
  <ScrollView contentContainerStyle={s.page}>
   ...
   {tab==='calendar'&&<Card title='التقويم القمري–الشمسي'><View onStartShouldSetResponder={()=>true} onResponderGrant={(e)=>setSwipeStartX(e.nativeEvent.pageX)} onResponderRelease={(e)=>{if(swipeStartX===null)return;const dx=e.nativeEvent.pageX-swipeStartX;if(dx>50)setCalendarDate(d=>addLunisolarMonths(d,-1));if(dx<-50)setCalendarDate(d=>addLunisolarMonths(d,1));setSwipeStartX(null)}}><Text style={s.hdate}>{calendarView.day} {calendarView.monthNameAr} {calendarView.year}</Text><View style={s.wrap}><Pressable style={s.choice} onPress={()=>setCalendarDate(d=>addLunisolarMonths(d,-1))}><Text style={s.choiceText}>الشهر السابق</Text></Pressable><Pressable style={s.choice} onPress={()=>setCalendarDate(new Date())}><Text style={s.choiceText}>اليوم</Text></Pressable><Pressable style={s.choice} onPress={()=>setCalendarDate(d=>addLunisolarMonths(d,1))}><Text style={s.choiceText}>الشهر التالي</Text></Pressable><Pressable style={s.choice} onPress={()=>setCalendarDate(d=>addLunisolarYears(d,-1))}><Text style={s.choiceText}>السنة السابقة</Text></Pressable><Pressable style={s.choice} onPress={()=>setCalendarDate(d=>addLunisolarYears(d,1))}><Text style={s.choiceText}>السنة التالية</Text></Pressable></View><View style={s.dayGrid}>{calendarDays.map(d=><Pressable key={d} style={[s.dayCell,d===calendarView.day&&s.dayCellOn]} onPress={()=>{const x=new Date(calendarDate);x.setUTCDate(x.getUTCDate()+(d-calendarView.day));setCalendarDate(x)}}><Text style={d===calendarView.day?s.dayTextOn:s.dayText}>{d}</Text></Pressable>)}</View><Text style={s.sub}>يمكنك التنقل بين الأيام والأشهر والسنوات السابقة واللاحقة.</Text></View></Card>}
   {tab==='events'&&<>
...
const s=StyleSheet.create({
 ...
 dayGrid:{flexDirection:'row',flexWrap:'wrap',gap:8,marginTop:12,justifyContent:'center'},dayCell:{width:'13%',minWidth:38,paddingVertical:10,borderRadius:10,borderWidth:1,borderColor:'#31536a',alignItems:'center'},dayCellOn:{backgroundColor:'#c89232',borderColor:'#c89232'},dayText:{color:'#eef4f7',fontWeight:'700'},dayTextOn:{color:'#071724',fontWeight:'900'},nav:{position:'absolute',bottom:0,left:0,right:0,height:70,backgroundColor:'#061520',borderTopWidth:1,borderTopColor:'#21465e',flexDirection:'row'},navb:{flex:1,alignItems:'center',justifyContent:'center'}
});

...
export function CopyrightScreen({rtl,onBack}){
 ...
 return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.content}>
  ...
  <Card title={ar?'علامات التحقق':'Verification markers'} rtl={rtl}><Paragraph rtl={rtl}>{ar?'تتضمن النسخة الرسمية باركود تعريف خاصًا بالنسخة يعرض اسم الأفق، صاحب المشروع، نوع النسخة، رقم الإصدار ومعرّف التثبيت. وعند ربط خادم التحقق الرسمي يمكن للباركود أن يحمل رابط تحقق ورمزًا قصير العمر للتحقق عبر الويب. هذه العلامات تساعد في التحقق من مصدر النسخة ولا تُعد وحدها ضمانًا قانونيًا للملكية.':'The official app includes a copy-identification QR containing the AlofoK name, project owner, edition, version and installation ID. When the official verification server is connected, the QR can carry a short-lived web verification link and code. These markers help verify the source of a copy but are not by themselves a legal guarantee of ownership.'}</Paragraph></Card>
 </ScrollView></SafeAreaView>
}
...
export function AuthenticityScreen({rtl,onBack,edition,version}){
 ...
 const localQrValue=challenge?.installationId?`AlofoK|owner=Wissam Mohammed - Wissam Digital|edition=${edition}|version=${version}|installation=${challenge.installationId}|type=official-copy-identity`:'';
 ...
 return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.content}>
  ...
  {!!qrValue&&<>
   ...
   <Text style={[s.expiry,dir(rtl)]}>{ar?'محتوى الباركود: الأفق / AlofoK — وسام محمد — Wissam Digital — نوع النسخة — رقم الإصدار — معرّف التثبيت.':'QR content: AlofoK — Wissam Mohammed — Wissam Digital — edition — version — installation ID.'}</Text>
  </>}

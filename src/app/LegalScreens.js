...
export function PrivacyScreen({rtl,onBack,onNavigate}){
 ...
 return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.content}>
  ...
  <Card title={ar?'طبيعة التطبيق':'About the app'} rtl={rtl}><Paragraph rtl={rtl}>{ar?'الأفق / AlofoK مشروع تقويمي وتقني وبحثي، وليس جهة دينية رسمية. تعرض النسختان التجريبية وPlus التقويمات والمواقيت والمناسبات والخدمات المرتبطة بها بحسب الميزات المتاحة في كل نسخة.':'AlofoK is a calendar, technology and research project, not an official religious authority. Trial and Plus provide calendars, prayer times, events and related services according to the features available in each edition.'}</Paragraph></Card>
...
export function CopyrightScreen({rtl,onBack}){
 ...
 return <SafeAreaView style={s.safe}><ScrollView contentContainerStyle={s.content}>
  ...
  <Card title={ar?'حقوق الأفق':'AlofoK rights'} rtl={rtl}><Paragraph rtl={rtl}>{ar?'© AlofoK / الأفق. جميع الحقوق محفوظة في الشفرة البرمجية الأصلية، تصميم الواجهات، الشعارات والأيقونات الأصلية، ترتيب المحتوى، النصوص والمواد التي أُنشئت خصيصًا للمشروع، وذلك إلى الحد الذي يسمح به القانون.':'© AlofoK. All rights are reserved in the original source code, interface design, original logos and icons, content arrangement, text and materials created specifically for the project, to the extent permitted by law.'}</Paragraph></Card>

...
function periodFromResponse(response){
 const data=response?.notification?.request?.content?.data;
 if(data?.kind!=='alofok-v3-adhkar')return null;
 return data?.period==='evening'?'evening':data?.period==='morning'?'morning':null;
}
...
export function useAdhkar({now=new Date(),fajrDate=null,timeZone='Asia/Baghdad',lat=33.3152,lon=44.3661,language='ar'}={}){
 ...
 useEffect(()=>{
  ...
  Notifications.getLastNotificationResponseAsync().then(response=>{
   if(!active)return;
   const period=periodFromResponse(response);
   if(period)setOpenedKind(period);
  }).catch(()=>{});
  return()=>{active=false};
 },[]);
   ...
   for(let i=0;i<14;i++){
     ...
     if(fajr){
      ...
      if(triggerDate.getTime()>Date.now()+5000){
       const id=await Notifications.scheduleNotificationAsync({
        content:{
         title:language.startsWith('ar')?'أذكار الصباح':'Morning adhkar',
         body:language.startsWith('ar')?'حان وقت أذكار الصباح. افتح الأفق للقراءة.':'It is time for morning adhkar. Open Al-Ufuq to read.',
         sound:'default',
         data:{kind:'alofok-v3-adhkar',period:'morning'}
        },
    ...
    if(wantsEvening){
     ...
     if(triggerDate.getTime()>Date.now()+5000){
      const id=await Notifications.scheduleNotificationAsync({
       content:{
        title:language.startsWith('ar')?'أذكار المساء':'Evening adhkar',
        body:language.startsWith('ar')?'حان وقت أذكار المساء. افتح الأفق للقراءة.':'It is time for evening adhkar. Open Al-Ufuq to read.',
        sound:'default',
        data:{kind:'alofok-v3-adhkar',period:'evening'}
       },
       trigger:{type:Notifications.SchedulableTriggerInputTypes.DATE,date:triggerDate,channelId:Platform.OS==='android'?channelId:undefined}
      });
      ids.push(id);
...
function SettingRow({rtl,title,hint,value,onChange}){
 return <View style={s.settingCard}>
  <View style={[s.settingRow,{flexDirection:rtl?'row-reverse':'row'}]}>
   ...
   <Switch value={value} onValueChange={onChange} trackColor={{false:'#38495B',true:'#A87B28'}} thumbColor={value?GOLD:'#E8EDF2'}/>
  </View>
 </View>
}
...
export function AdhkarScreen({kind,rtl,onBack,onComplete}){
 ...
 return <SafeAreaView style={s.safe}>
  <ScrollView contentContainerStyle={s.content}>
   ...
   <Text style={[s.intro,{textAlign:rtl?'right':'left'}]}>{rtl?'بعد إكمال القراءة اضغط «تمت القراءة — إخفاء»، ولن تعود البطاقة في نفس الفترة.':'When finished, tap “Finished — hide”; the card will not return in the same period.'}</Text>
   {items.map((item,index)=><View key={index} style={s.dhikrCard}>
    <View style={[s.dhikrTop,{flexDirection:rtl?'row-reverse':'row'}]}>
     <Text style={[s.dhikrTitle,{textAlign:rtl?'right':'left'}]}>{item.title}</Text>
...
const s=StyleSheet.create({
 ...
 header:{height:60,alignItems:'center',justifyContent:'space-between'},
 ...
 intro:{color:MUTED,fontSize:12,lineHeight:19,marginVertical:8},
 homeCard:{marginTop:12,minHeight:72,borderRadius:16,borderWidth:1,borderColor:'rgba(244,196,93,.55)',backgroundColor:'rgba(7,23,41,.88)',paddingHorizontal:14,flexDirection:'row',alignItems:'center',gap:12},
 homeIcon:{width:40,height:40,borderRadius:20,backgroundColor:'rgba(244,196,93,.14)',borderWidth:1,borderColor:GOLD,alignItems:'center',justifyContent:'center'},
 homeIconText:{color:GOLD,fontSize:21},
 homeTitle:{color:GOLD,fontSize:16,fontWeight:'900'},
 homeHint:{color:MUTED,fontSize:11,marginTop:4},
 chevron:{color:WHITE,fontSize:25},
 settingsWrap:{gap:8,marginTop:10},
 settingsHeading:{color:GOLD,fontSize:17,fontWeight:'900',marginTop:4},
 settingCard:{backgroundColor:CARD,borderRadius:15,borderWidth:1,borderColor:LINE,padding:13},
 settingRow:{alignItems:'center',gap:12},
 settingTitle:{color:WHITE,fontSize:14,fontWeight:'800'},
 settingHint:{color:MUTED,fontSize:11,marginTop:4},
 dhikrCard:{backgroundColor:CARD,borderRadius:17,borderWidth:1,borderColor:LINE,padding:15,marginTop:12},
 dhikrTop:{alignItems:'center',justifyContent:'space-between',gap:8},
 dhikrTitle:{color:GOLD,fontSize:16,fontWeight:'900',flex:1},
 count:{color:MUTED,fontSize:11},
 dhikrText:{color:WHITE,fontSize:18,lineHeight:34,textAlign:'right',writingDirection:'rtl',marginTop:12},
 doneButton:{backgroundColor:GOLD,borderRadius:15,paddingVertical:15,alignItems:'center',marginTop:18},

...
export function useAdhkar({now=new Date(),fajrDate=null,timeZone='Asia/Baghdad',lat=33.3152,lon=44.3661,language='ar'}={}){
 ...
 useEffect(()=>{
  ...
  Notifications.getLastNotificationResponseAsync().then(readResponse).catch(()=>{});
  const sub=Notifications.addNotificationResponseReceivedListener(readResponse);
  return()=>sub.remove();
 },[]);

 const today=zoneParts(now,timeZone).key;

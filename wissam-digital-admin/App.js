 ...
 return <SafeAreaView style={s.safe}>
  ...
  <ScrollView contentContainerStyle={s.content} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={()=>{setRefreshing(true);load(true)}} tintColor={GOLD}/>}>
   ...
   <View style={s.tabs}>
    ...
    <Pressable onPress={()=>setActiveTab('approved')} style={[s.tab,activeTab==='approved'&&s.tabActive]}><Text style={[s.tabText,activeTab==='approved'&&s.tabTextActive]}>مقبول ({approved})</Text></Pressable>
    <Pressable onPress={()=>setActiveTab('rejected')} style={[s.tab,activeTab==='rejected'&&s.tabActive]}><Text style={[s.tabText,activeTab==='rejected'&&s.tabTextActive]}>مرفوض ({rejected})</Text></Pressable>
   </View>

...
const [eventPack,setEventPack]=useState('common');
 ...
 return <SafeAreaView style={s.root}>
  ...
  <View style={s.nav}>{[['calendar','التقويم'],['adhan','الأذان'],['settings','الإعدادات']].map(([id,t])=><Pressable key={id} onPress={()=>setTab(id)} style={s.navb}><Text style={tab===id?s.active:s.muted}>{t}</Text></Pressable>)}</View>
 </SafeAreaView>
}
